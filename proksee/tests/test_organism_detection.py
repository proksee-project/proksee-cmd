...
class TestReforg():
    ...
    def test_refseq_string_good(self):
        method_string_good = \
            organism_identify_good._OrganismDetection__refseq_masher_string()
        assert refseq_string_good == method_string_good

    # Test for negating bad refseq_masher string
    def test_refseq_string_bad(self):
        refseq_string_bad = 'refseq_masher matches(deleted whitespace)' + \
            forward_bad
        method_string = \
            organism_identify_bad._OrganismDetection__refseq_masher_string()
    ...
    def test_refseq_func_badparams(self):
        ...
        with pytest.raises(subprocess.CalledProcessError):
            assert \
                organism_identify_good._OrganismDetection__refseq_masher_func(
                    refseq_string_test)
    ...
    def test_refseq_func_badfile(self):
        ...
        with pytest.raises(subprocess.CalledProcessError):
            assert \
                organism_identify_bad._OrganismDetection__refseq_masher_func(
                    refseq_string_bad)
    ...
    def test_identify_organism_good(self):
        method_string = \
            organism_identify_good._OrganismDetection__identify_organism(
                refseq_out_good)
    ...
    def test_identify_organism_drafted_solo(self):
        refseq_out_bad = \
            os.path.join(TEST_INPUT_DIR, 'refseq_fancysnakes1.out')
        method_string = \
            organism_identify_good._OrganismDetection__identify_organism(
                refseq_out_bad)
    ...
    def test_identify_organism_drafted_tie(self):
        refseq_out_bad = \
            os.path.join(TEST_INPUT_DIR, 'refseq_fancysnakes2.out')
        method_string = \
            organism_identify_good._OrganismDetection__identify_organism(
                refseq_out_bad)
        str1 = 'Madame White (probability : 0.22), '
        str2 = 'Master Viper (probability : 0.22), '
        expected_string = str1 + str2
        assert expected_string == method_string

    # Test for OrganismDetection class method integrating all methods
    def test_major_organism_good(self):
        refseq_output_string = 'Major reference organism is/are ' + \
            'Escherichia coli (probability : 1.0), . Return code ' + \
            str(refseq_func_good_rc)
